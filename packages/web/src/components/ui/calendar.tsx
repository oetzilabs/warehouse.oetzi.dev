...
import { Show, splitProps, type Component, type ComponentProps, type JSX, type ValidComponent } from "solid-js";

const Calendar = CalendarPrimitiveRoot;

const CalendarHead: Component<ComponentProps<"div">> = (props) => {
  const [local, others] = splitProps(props, ["class"]);
  return <div class={cn("relative flex w-full items-center justify-between pt-1 pb-4", local.class)} {...others} />;
};
...
const CalendarPrevTrigger = <T extends ValidComponent = "button">(props: DynamicProps<T, CalendarTriggerProps<T>>) => {
...
const CalendarNextTrigger = <T extends ValidComponent = "button">(props: DynamicProps<T, CalendarTriggerProps<T>>) => {
  const [local, others] = splitProps(props as CalendarTriggerProps, ["class", "children"]);
  return (
    <CalendarPrimitive.Nav
      action="next-month"
...
const CalendarHeading = <T extends ValidComponent = "h2">(props: DynamicProps<T, CalendarHeadingProps<T>>) => {
...
const CalendarGrid = <T extends ValidComponent = "table">(props: DynamicProps<T, CalendarGridProps<T>>) => {
  const [local, others] = splitProps(props as CalendarGridProps, ["class"]);
  return <CalendarPrimitive.Table class={cn("w-full border-collapse space-x-1", local.class)} {...others} />;
};
