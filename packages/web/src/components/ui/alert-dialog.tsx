...
const AlertDialogClose = <T extends ValidComponent = "button">(
  props: PolymorphicProps<T, AlertDialogCloseButtonProps<T>>,
) => {
  const [local, others] = splitProps(props as AlertDialogCloseButtonProps, ["class"]);
  return (
    <AlertDialogPrimitive.CloseButton
      class={cn("rounded-sm opacity-70 ring-offset-background", local.class)}
      {...others}
    />
  );
};

type AlertDialogActionButtonProps<T extends ValidComponent = "button"> =
  AlertDialogPrimitive.AlertDialogCloseButtonProps<T> & {
    class?: string | undefined;
  };

const AlertDialogAction = <T extends ValidComponent = "button">(
  props: PolymorphicProps<T, AlertDialogActionButtonProps<T>>,
) => {
  const [local, others] = splitProps(props as AlertDialogActionButtonProps, ["class"]);
  return (
    <AlertDialogPrimitive.CloseButton
      data-type="action"
      class={cn("rounded-sm opacity-70 ring-offset-background", local.class)}
