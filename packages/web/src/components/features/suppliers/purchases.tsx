...
type PurchasesProps = {
  list: Accessor<SupplierInfo["purchases"]>;
};
  ...
  return (
    <div class="flex flex-col border rounded-lg overflow-clip">
      <div class="flex flex-row items-center gap-2 justify-between p-4 py-2 pr-2 border-b bg-muted-foreground/5 dark:bg-muted/30">
        <h2 class="font-medium">Purchases</h2>
      ...
      <div class="flex flex-col w-full">
        ...
        <Show when={props.list().length > 0}>
          <div class="flex flex-col gap-0">
            <For each={props.list().slice(0, 10)}>
              {(o) => (
            ...
            <Show when={props.list().length > 10}>
              <div class="flex flex-row items-center justify-center p-4 border-t bg-muted/30">
                <Button as={A} href="./orders" variant="ghost" size="sm" class="gap-2">
                  Show All Orders ({props.list().length})
                  <ArrowUpRight class="size-4" />
                </Button>
              </div>
            </Show>
