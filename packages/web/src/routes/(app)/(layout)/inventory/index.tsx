...
const ReorderDialog = (props: { product: InventoryAlertInfo[number]["product"] }) => {
  ...
  const lastSupplyOrder = (o: InventoryAlertInfo[number]["product"]["orders"][number]) => {
    ...
    const earliestSchedule = supplier.schedules
      .filter((s) => s.type === "delivery" && dayjs(s.schedule.scheduleStart).isAfter(dayjs()))
      .sort((a, b) => dayjs(a.schedule.scheduleStart).diff(dayjs(b.schedule.scheduleStart)))[0];

    if (!earliestSchedule) return dayjs().add(1, "day").day();

    return dayjs(earliestSchedule.schedule.scheduleStart).day();
  };
